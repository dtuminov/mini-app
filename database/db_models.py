...
class User(BaseModel):
    ...
    register_date = Column(DateTime(timezone=True), nullable=False)  # Изменено
    ...
    upd_date = Column(DateTime(timezone=True), nullable=True)  # Изменено
    freeze = Column(Integer, nullable=False, default=0)
    promocodes = Column(ARRAY(String), nullable=False, default=[])

    # jumanji
    jumanji_attempts = Column(JSON, nullable=False,
                            default={"dice": 1, "freecard": 1, "box": 1,
                                    "casino": 1, "basket": 1, "darts": 1,
                                    "bowling": 1})
...
class Transaction(BaseModel):
    ...
    transaction_date = Column(DateTime(timezone=True), nullable=False)  # Изменено
