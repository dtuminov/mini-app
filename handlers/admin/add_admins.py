...
# loader
def load_admin_add_admins_handler(dispatcher: Dispatcher, bot: Bot):
	...
	dispatcher.message.register(
		add_admins_choose,
		F.Text(equals=["👮‍♂️ Управление админами", "🔚 Назад"]),
		StateFilter(AdminStates.main_menu, AdminStates.add_admin_ADD, AdminStates.add_admin_DEL)
	)

	## ADD ADMIN
	dispatcher.message.register(
		add_admins_ADD_1,
		F.Text(equals=["➕ Добавить"]),
		StateFilter(AdminStates.add_admin_choose)
	)

	dispatcher.message.register(
		add_admins_ADD_2,
		F.content_types == "text",  # Альтернативно можно использовать filters.ContentTypesFilter([ContentType.TEXT])
		StateFilter(AdminStates.add_admin_ADD)
	)

	## DEL ADMIN
	dispatcher.message.register(
		add_admins_DEL_1,
		F.Text(equals=["➖ Убрать"]),
		StateFilter(AdminStates.add_admin_choose)
	)
	...
	dispatcher.message.register(
		add_admins_DEL_2,
		F.content_types == "text",  # Альтернативно можно использовать filters.ContentTypesFilter([ContentType.TEXT])
		StateFilter(AdminStates.add_admin_DEL)
	)
