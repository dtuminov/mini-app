...
# loader
def load_sp_trade_handler(dispatcher: Dispatcher):
	...
	dispatcher.register_message_handler(
		...
		state = MainStates.enter_trade_link
	)
	...
	dispatcher.register_callback_query_handler(
		...
		lambda x: x.data.startswith("acceptTrade_cards_deny_") or x.data.startswith("sendTrade_cards_deny_") or x.data.startswith("sendTrade_deny_"),
		state = "*"
	)
...
async def process_streamPlace_sendTrade(query: types.CallbackQuery, state: FSMContext):
	...
	async with state.proxy() as storage:
		storage["sendTrade_msgID"] = query.message.message_id

	await MainStates.enter_trade_link.set()
...
async def process_streamPlace_sendTradeChooseCard_msg(message: types.Message, state: FSMContext):
	...
	# unpack
	async with state.proxy() as storage:
		last_msg_id = storage.get("sendTrade_msgID")
	...
	async with state.proxy() as storage:
		storage["sendTrade_target_username"] = target_username

	await message.answer(
		text = "💬 Выберите карту, которую хотите отдать:"
	)
...
🃏 Количество: {my_cards_by_rarity.count(card_to_show.card_id)}"""
	...
	await message.answer_photo(
		...
		caption = caption,
		reply_markup = get_opened_card_ikb(offset = offset, max_offset = len(my_cards_by_rarity_unique), choose_btn = True, current_card_id = card_to_show.card_id, prefix = "sendTrade_cards")
	)
...
async def process_streamPlace_sendTradeChooseCard_query(query: types.CallbackQuery, state: FSMContext):
	...
	async with state.proxy() as storage:
		offset = storage.get("sendTrade_offset", 0)
	...
	if offset >= len(my_cards_by_rarity_unique):
		offset = 0
		async with state.proxy() as storage:
			storage["sendTrade_offset"] = offset
...
🃏 Количество: {my_cards_by_rarity.count(card_to_show.card_id)}"""
	...
	await query.message.edit_media(media = types.InputMediaPhoto(media = card_to_show.card_image, caption = caption), reply_markup = get_opened_card_ikb(offset = offset, max_offset = len(my_cards_by_rarity_unique), prefix = "sendTrade_cards", current_card_id = card_to_show.card_id, choose_btn = True))
...
async def process_streamPlace_sendTradeChooseCard_confirm(query: types.CallbackQuery, state: FSMContext):
	...
	async with state.proxy() as storage:
		target_username = storage.get("sendTrade_target_username")
...
async def process_streamPlace_acceptTradeChooseCard(query: types.CallbackQuery, state: FSMContext):
	...
	if query.data.startswith("accept_trade_"):
		...
		async with state.proxy() as storage:
			storage["acceptTrade_offset"] = 0
			storage["acceptTrade_card_id"] = int(query.data.split("_")[-1])
			storage["acceptTrade_user_id"] = int(query.data.split("_")[-2])
		
		offset = 0
	else:
		async with state.proxy() as storage:
			offset = storage.get("acceptTrade_offset", 0)

	await query.answer(
		text = "💬 Выберите карту для встречного предложения:"
	)
	...
	if offset >= len(my_cards_by_rarity):
		offset = 0
		async with state.proxy() as storage:
			storage["acceptTrade_offset"] = offset
...
🃏 Количество: {my_cards_by_rarity.count(card_to_show.card_id)}"""
	...
	async with state.proxy() as storage:
		target_id = storage.get("acceptTrade_user_id")
...
async def process_streamPlace_acceptTradeChooseCard_confirm(query: types.CallbackQuery, state: FSMContext):
	...
	async with state.proxy() as storage:
		target_id = storage.get("acceptTrade_user_id")
		target_card_id = storage.get("acceptTrade_card_id")
...
🔄 Встречное предложение от <a href='https://t.me/{query.from_user.username}'>{query.from_user.full_name.replace("<", "&lt;").replace(">", "&gt;")}</a> на вашу <b>{target_card.card_name} ({target_card.card_rarity})</b> 🔄"""
	...
	try:
		await g_bot.send_photo(
			...
			photo = card_to_show.card_image,
			caption = caption,
			reply_markup = get_trade2_ikb(user_id = query.from_user.id, target_card_id = target_card_id, card_id = card_id, target_user_id = query.from_user.id)
		)
	except Exception as e:
		print(e)
