...
# loader
def load_admin_search_user_handler(dispatcher: Dispatcher):
	...
	dispatcher.register_message_handler(
		...
		filters.Text(equals = "🔎 Поиск пользователя"),
	...
	dispatcher.register_message_handler(
		...
		state = AdminStates.search_user_enterEntity
	)

	# back
	dispatcher.register_callback_query_handler(
		search_user_getUser_query,
		lambda x: x.data.startswith("back_to_su_"),
		state = [
	...
	dispatcher.register_callback_query_handler(
		...
		lambda x: x.data.startswith("su_edit_attempts_"),
		state = AdminStates.search_user_actions
	)

	dispatcher.register_message_handler(
		search_user_addAttempts_2,
		content_types = ContentType.TEXT,
		state =  AdminStates.search_user_enterAttempts
	)

	# freeze
	dispatcher.register_callback_query_handler(
		search_user_addFreeze_1,
		lambda x: x.data.startswith("su_edit_freeze_"),
		state = AdminStates.search_user_actions
	)
	...
	dispatcher.register_message_handler(
		...
		content_types = ContentType.TEXT,
		state =  AdminStates.search_user_enterFreeze
	)

	# card
	dispatcher.register_callback_query_handler(
		search_user_addCard_1,
		lambda x: x.data.startswith("su_add_card_"),
		state = AdminStates.search_user_actions
	)
	...
	dispatcher.register_message_handler(
		...
		content_types = ContentType.TEXT,
		state =  AdminStates.search_user_enterCard
	)

	# del card
	dispatcher.register_callback_query_handler(
		search_user_delCard_1,
		lambda x: x.data.startswith("su_del_card_"),
		state = AdminStates.search_user_actions
	)
	...
	dispatcher.register_message_handler(
		...
		content_types = ContentType.TEXT,
		state =  AdminStates.search_user_enterdelCard
	)

	### trans list
	dispatcher.register_callback_query_handler(
		process_transactions_list,
		lambda x: x.data.startswith("su_transactions_"),
		state = "*"
	)
